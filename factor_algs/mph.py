...
class CCfgFactorGrpMPH(CCfgFactorGrpWinLambda):
    def __init__(self, **kwargs):
        super().__init__(factor_class="MPH", **kwargs)
...
class CFactorMPH(CFactorsByInstru):
    def __init__(self, factor_grp: CCfgFactorGrpMPH, **kwargs):
        if not isinstance(factor_grp, CCfgFactorGrpMPH):
            raise TypeError("factor_grp must be CCfgFactorGrpMPH")
    ...
    def cal_factor_by_instru(self, instru: str, bgn_date: str, stp_date: str, calendar: CCalendar) -> pd.DataFrame:
        ...
        pos_lng_hld_by_c = pos_lng_data.groupby(by=["trade_date", "ts_code"]).apply(self.cal_top, x="long_hld")
        pos_srt_hld_by_c = pos_srt_data.groupby(by=["trade_date", "ts_code"]).apply(self.cal_top, x="short_hld")
        pos_lng_hld: pd.DataFrame = pos_lng_hld_by_c.groupby(by="trade_date").sum()
        pos_srt_hld: pd.DataFrame = pos_srt_hld_by_c.groupby(by="trade_date").sum()

        res = {}
        for lbd in self.cfg.lbds:
            top = int(lbd * self.TOT_SIZE)
            name_top = self.cfg.name_top(top)
            d0: pd.Series = pos_lng_hld[name_top] - pos_srt_hld[name_top]
            d1: pd.Series = pos_lng_hld[name_top] + pos_srt_hld[name_top]
