...
def reformat(raw_data: pd.DataFrame) -> pd.DataFrame:
    return raw_data.rename(
        columns={"return_c_major": "return", "amount_major": "amount"}
...
def get_available_universe(
    ...
    avlb_ret_data = return_df[filter_df].stack().reset_index().rename(columns={0: "return", "level_1": "instrument"})
    avlb_amt_data = amt_df[filter_df].stack().reset_index().rename(columns={0: "amount", "level_1": "instrument"})
    update_df = pd.merge(
        left=avlb_ret_data,
        right=avlb_amt_data,
        how="inner",
        on=["trade_date", "instrument"],
    ).sort_values(by=["trade_date", "amount"], ascending=[True, False])
