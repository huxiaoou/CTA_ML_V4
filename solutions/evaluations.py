...
def main_evl_tests(tests: list[CTestMclrn], sim_save_dir: str, evl_save_dir: str):
    summary_all = []
    for test in track(tests, description="Evaluating tests"):
        d = evl_sim(sim_id=test.save_id, sim_save_dir=sim_save_dir, evl_save_dir=evl_save_dir)
        args_model = test.test_model.to_dict()
        del args_model["hyper_param_grids"]
        args_data = {
            "ret": test.test_data.ret.ret_name,
            "ret_type": test.test_data.ret_type,
            "factor_type": test.test_data.factor_type,
        }
        d.update({**args_model, **args_data})
