...
class CCfgFactorGrpISKEW(CCfgFactorGrpWin):
    def __init__(self, **kwargs):
        super().__init__(factor_class="ISKEW", **kwargs)
...
class CFactorISKEW(CFactorsByInstru):
    def __init__(self, factor_grp: CCfgFactorGrpISKEW, **kwargs):
        if not isinstance(factor_grp, CCfgFactorGrpISKEW):
            raise TypeError("factor_grp must be CCfgFactorGrpISKEW")
    ...
    def cal_factor_by_instru(self, instru: str, bgn_date: str, stp_date: str, calendar: CCalendar) -> pd.DataFrame:
        ...
        iskew = minb_data.groupby(by="trade_date")["simple"].skew()
        for win, name_vanilla in zip(self.cfg.wins, self.cfg.names_vanilla):
            maj_data[name_vanilla] = iskew.rolling(win).sum()
        w0, w1 = 240, 20
